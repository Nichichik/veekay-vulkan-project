...
layout (binding = 3) uniform sampler2D albedo_textures[8];
...
vec3 calculate_point_light(PointLight light, vec3 normal, vec3 frag_pos, vec3 view_dir, vec3 base_color) {
    ...
    float attenuation = 1.0 / (1 + light.linear * distance + light.quadratic * (distance * distance));//коэффициент затухания
...
void main() {
   ...
   vec3 view_dir = normalize(camera_position - f_position_world);
       ...
       if (use_texture > 0.5) {
           ...
           base_color = texture(albedo_textures[index], f_uv).rgb;
       } else {
