...
namespace {
    ...
    struct Camera {
        constexpr static float default_fov = 60.0f;//поле зрения по вертикали
        constexpr static float default_near_plane = 0.01f;
        constexpr static float default_far_plane = 100.0f;

        veekay::vec3 position = {};
        float yaw = 90.0f; // Поворот влево-вправо
        float pitch = 0.0f;  // Поворот вверх-вниз
...
// NOTE: Scene objects
    inline namespace {
        ...
        enum class CameraMode { LookAt, TransformInverse };
        ...
        veekay::vec3 look_at_target_point = {0.0f, 0.5f, 0.0f};
        veekay::vec3 look_at_world_up = {0.0f, 1.0f, 0.0f};
...
// NOTE: Vulkan objects
    ...
    //векторы камеры на основе yaw и pitch
    ...
    veekay::vec3 Camera::get_right_vector() const {//правило правой руки
...
// NOTE: Your shaders are compiled via CMake with this code too, look it up
    ...
    void initialize(VkCommandBuffer cmd) {
        ...
        {
            {
                ...
                if (lodepng::decode(image_data, width, height, "shaders/texture.png")) {
                    textures.push_back(missing_texture);
                } else {
                    textures.push_back(new veekay::graphics::Texture(cmd, width, height, VK_FORMAT_R8G8B8A8_UNORM, image_data.data()));
                }
            }

            {
                std::vector<unsigned char> image_data;
                unsigned int width, height;
                if (lodepng::decode(image_data, width, height, "shaders/texture_cube_red.png")) {
                    textures.push_back(missing_texture);
                } else {
                    textures.push_back(new veekay::graphics::Texture(cmd, width, height, VK_FORMAT_R8G8B8A8_UNORM, image_data.data()));
                }
...

            {
                std::vector<unsigned char> image_data;
                unsigned int width, height;
                if (lodepng::decode(image_data, width, height, "shaders/texture_cube_green.png")) {
                    textures.push_back(missing_texture);
                } else {
                    textures.push_back(new veekay::graphics::Texture(cmd, width, height, VK_FORMAT_R8G8B8A8_UNORM, image_data.data()));
                }
            ...
            while (textures.size() < max_textures) {
                textures.push_back(missing_texture);
                samplers.push_back(missing_texture_sampler);
            }
        }
        {
...

        {
            ...
            corner_walls_mesh.vertex_buffer = new veekay::graphics::Buffer(
                    vertices.size() * sizeof(Vertex), vertices.data(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
            corner_walls_mesh.index_buffer = new veekay::graphics::Buffer(
                    indices.size() * sizeof(uint32_t), indices.data(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
            corner_walls_mesh.indices = uint32_t(indices.size());
        }
        ...
        saved_lookat_state = initial_state;
        saved_transform_state = initial_state;
...
// NOTE: Destroy resources here, do not cause leaks in your program!
    void shutdown() {
        ...
        delete corner_walls_mesh.index_buffer;
        delete corner_walls_mesh.vertex_buffer;
    ...
    void update(double time) {
        ...
        const char* modes[] = { "Look-At (Basis Vectors)", "Transform Inverse" };
        int current_mode_index = static_cast<int>(current_camera_mode);
        ...
        if (!ImGui::IsWindowFocused(ImGuiFocusedFlags_AnyWindow)) {
            ...
            veekay::vec3 move_front;
            veekay::vec3 move_right;
            move_front = camera.get_front_vector();
            move_right = camera.get_right_vector();
